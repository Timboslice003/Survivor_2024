...
#Create list of teams used
teams_used <- c()

#Create empty tibble for the dataframe
favs_once <- tibble()

for (i in 1:18){
  temp_df <- week_odds_dv %>% 
    filter(week == i) %>% 
    filter(!team %in% teams_used) %>% 
    filter(team_wp == max(team_wp))
  
  teams_used[i] <- temp_df$team
...
#Weeks 1-9 drop opponent win probability
first_9_df <- favs_once %>%
  filter(week < 10) %>% 
...
first_9_df %>%
  ...
  gt::tab_header(title = '2023 NFL Weekly Favorites',
                 subtitle = 'Team Can Only Be Used Once') %>%
